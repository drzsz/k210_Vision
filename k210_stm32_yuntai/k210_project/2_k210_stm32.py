...
def sending_data(x, y):
    """发送坐标到串口"""
    # 确保值在0-255范围内
    x_val = max(0, min(255, x))
    y_val = max(0, min(255, y))
    # 发送原始值
    #FH = b'{},{}'.format(x_val,y_val)
    #uart.write(FH)
    #print("Sent: x=%d, y=%d" % (x_val, y_val))
#=================================
...
#def sending_data(x, y):
    # 坐标映射到0-255范围 (QVGA分辨率 320x240)
    #x_val = max(0, min(255, int(x * 255 / 319)))
    # y_val = max(0, min(255, int(y * 255 / 239)))

#====通过电脑的虚拟UART进行传输，能在stm32的oled上显示坐标=====
    #x_val = max(0, min(320, x))
    #y_val = max(0, min(240, y))
...
#=======用物理UART脱机传输，暂时未成功=======
    ...
    # 发送二进制协议：0xAA + X + Y + 校验和
    #checksum = (0xAA + x_val + y_val) & 0xFF
    #uart.write(bytearray([0xAA, x_val, y_val, checksum]))

    # 返回要显示的数据字符串
    #return f"TX: 0xAA,{x_val},{y_val},{checksum}"
